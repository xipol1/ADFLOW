...
class ApiService {
  ...
  // ==========================================
  // ONBOARDING — VERIFICACIÓN REAL POR PLATAFORMA
  // ==========================================

  async onboardingTelegramInstructions(chatId) {
    return this.request('/onboarding/telegram/instrucciones', { method: 'POST', body: JSON.stringify({ chatId }) });
  }

  async onboardingTelegramVerify(chatId, canalId) {
    return this.request('/onboarding/telegram/verificar', { method: 'POST', body: JSON.stringify({ chatId, canalId }) });
  }

  async onboardingDiscordInstructions(guildId) {
  ...
  async onboardingInstagramAuthUrl(canalId, source) {
    ...
    const qs = params.toString();
    return this.request(`/onboarding/instagram/auth-url${qs ? `?${qs}` : ''}`);
  }

  async onboardingWhatsappInstructions(channelId, creatorPhone, canalId) {
    return this.request('/onboarding/whatsapp/instrucciones', {
      method: 'POST',
      body: JSON.stringify({ channelId, creatorPhone, canalId }),
    });
  }

  async onboardingWhatsappCheckOTP(verificacionId) {
    return this.request(`/onboarding/whatsapp/check-otp/${verificacionId}`);
  }

  async onboardingWhatsappVerifyOTPManual(verificacionId, codigo) {
    return this.request('/onboarding/whatsapp/verificar-otp', {
      method: 'POST',
      body: JSON.stringify({ verificacionId, codigo }),
    });
  }

  async onboardingWhatsappVerify(verificacionId, canalId, seguidoresDeclarados) {
  ...
  async onboardingChannelStatus(canalId) {
    return this.request(`/onboarding/estado/${canalId}`);
  }
