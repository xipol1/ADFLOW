  ...
  useEffect(() => {
    ...
    apiService.verifyEmail(token).then(res => {
      if (res?.success) {
        ...
        // Creadores recién verificados siguen su onboarding; anunciantes van al dashboard.
        const rol = res.user?.rol || res.user?.role || ''
        const isCreator = rol === 'creador' || rol === 'creator'
        const destination = isCreator ? '/onboarding/channel' : '/dashboard'
