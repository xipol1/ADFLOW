...
export default function ChannelDetailPage() {
      ...
      try {
        ...
        if (!cancelled) {
          if (res?.success && res.data) {
            ...
            setChannel(null)
            setError(res?.message || 'No se encontró el canal')
          }
        }
      } catch (err) {
        if (!cancelled) {
  ...
  // --- Error / not found state (no mock fallback) ---
  if (!channel) {
    return (
      <div style={{ background: 'var(--bg)', color: 'var(--text)', fontFamily: F, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center', maxWidth: '420px', padding: '0 24px' }}>
          ...
          <p style={{ fontSize: '14px', color: 'var(--muted)', marginBottom: '24px' }}>
            No pudimos cargar este canal.
          </p>
          <Link to="/marketplace" style={{
            background: PURPLE, color: '#fff', borderRadius: '10px',
            padding: '10px 24px', fontSize: '14px', fontWeight: 600,
            textDecoration: 'none', fontFamily: F, display: 'inline-block',
          }}>
  ...
  const data = channel
  const nombre = data.nombre || data.name || 'Canal'
  const plataforma = (data.plataforma || data.platform || 'whatsapp').toLowerCase()
  ...
  const miembros = data.miembros || data.audience || data.audiencia || 0
  const precio = data.precio || data.pricePerPost || 0
  const rating = data.rating || 0
  const reviews = data.reviews || 0
  const engagement = data.engagement || '—'
  const categoria = data.categoria || data.category || 'General'
  const verificado = data.verificado || data.verified || false
