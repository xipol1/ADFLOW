...
router.get('/stats', autenticar, async (req, res) => {
  try {
    ...
    const referrals = await Usuario.find({ referredBy: user._id })
      .select('nombre email emailVerificado rol createdAt ultimaActividad')
      .lean()

    // Get GMV per referral + milestones
    const Campaign = require('../models/Campaign')
    const Canal = require('../models/Canal')
    const referralStats = await Promise.all(referrals.map(async (ref) => {
      // Campaigns where referral is advertiser OR creator (channel owner)
      const asAdvertiser = await Campaign.find({ advertiser: ref._id, status: 'COMPLETED' }).lean()
      const gmvAdvertiser = asAdvertiser.reduce((sum, c) => sum + (c.price || 0), 0)

      // Count all campaigns (any status) to know if they created one
      const allCampaigns = await Campaign.countDocuments({
        $or: [{ advertiser: ref._id }, { creator: ref._id }],
      })
      const paidCampaigns = await Campaign.countDocuments({
        advertiser: ref._id,
        status: { $in: ['PAID', 'PUBLISHED', 'COMPLETED'] },
      })

      // Check if referral has channels (for creators)
      const channelCount = await Canal.countDocuments({ propietario: ref._id })

      // Milestones
      const milestones = {
        registered: true,
        emailVerified: ref.emailVerificado === true,
        hasChannel: channelCount > 0,
        createdCampaign: allCampaigns > 0,
        paidCampaign: paidCampaigns > 0,
        completedCampaign: asAdvertiser.length > 0,
      }

      // Determine status from milestones
      let status = 'registered'
      if (milestones.completedCampaign) status = 'active'
      else if (milestones.paidCampaign) status = 'converting'
      else if (milestones.createdCampaign || milestones.hasChannel) status = 'engaged'
      else if (milestones.emailVerified) status = 'verified'
      ...
      return {
        ...
        email: ref.email,
        rol: ref.rol,
        gmvGenerated: gmvAdvertiser,
        active: status === 'active' || status === 'converting',
        status,
        milestones,
        joinedAt: ref.createdAt,
        lastActive: ref.ultimaActividad || ref.createdAt,
      }
    }))
