 ...
 * 2.  Auto-complete campaigns published > 48 hours with no action
 * 3.  Release escrow for completed campaigns
 *
 * Auto-complete window: 48 hours from PUBLISHED → COMPLETED. Industry
 * standard for short-form social posts (Telegram pins, IG stories, etc.).
 * Snapshots run by campaignId y continúan después de COMPLETED, así que
 * el tracking link data se preserva.
 *
...
async function runCampaignAutomation() {
  try {
    ...
    // 2. Auto-complete published campaigns older than the AUTO_COMPLETE
    // window. Reducido de 7 días a 48 horas para que los creators vean su
    // payout rápido tras publicarse — el escrow ya protege al advertiser
    // y las disputas tienen su propio estado.
    ...
    if (autoCompleted.modifiedCount > 0) {
      console.log(`[cron] Auto-completed ${autoCompleted.modifiedCount} campaigns (published > ${AUTO_COMPLETE_HOURS}h)`);
    }
        ...
        try {
          ...
          if (pi.status === 'requires_capture') {
            await stripe.paymentIntents.capture(campaign.stripePaymentIntentId, {
              idempotencyKey: `capture:${campaign.stripePaymentIntentId}`,
            });
    ...
    // 8. Cleanup stale Baileys pending_qr sessions.
    //
    // Only meaningful when the BaileysSessionManager was instantiated in this
    // process (i.e. on Fly/VPS where someone scanned a QR). On Vercel the
    // manager is never loaded, so we lazy-require it and swallow the load
    // failure — there's nothing to clean anyway.
    try {
      const baileysManager = require('../services/baileys/BaileysSessionManager');
      const expired = await baileysManager.cleanupStalePendingSessions();
      if (expired > 0) {
        console.log(`[cron] Baileys cleanup: expired ${expired} stale pending_qr sessions`);
      }
    } catch (baileysErr) {
      // Acceptable on Vercel — @whiskeysockets/baileys may not be in the
      // serverless bundle and the manager throws on require.
      if (!/baileys/i.test(baileysErr.message)) {
        console.error('[cron] Baileys cleanup error:', baileysErr.message);
      }
    }
