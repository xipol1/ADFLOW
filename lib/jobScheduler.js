...
async function runJob(job, { force = false } = {}) {
  ...
  if (!(await ensureDb())) return { type: job.type, status: 'skipped', reason: 'no-db' };

  // Window guard: already completed inside the dedup window? skip.
  // `force` (manual CLI / GitHub Action runs) bypasses it; the overlap guard
  // below still prevents a genuinely concurrent duplicate.
  if (!force) {
    ...
    if (recent) return { type: job.type, status: 'skipped', reason: 'window' };
  }
  ...
  if (inflight) return { type: job.type, status: 'skipped', reason: 'inflight' };
  ...
  try {
    ...
    if (err.code === 11000) return { type: job.type, status: 'skipped', reason: 'collision' };
    console.error(`[scheduler] ${job.type}: could not create JobLog:`, err.message);
    return { type: job.type, status: 'skipped', reason: 'joblog-error' };
  }
  ...
  try {
    ...
    return { type: job.type, status: 'completed', ms, result: summarize(result) };
  } catch (err) {
    await JobLog.updateOne(
      { _id: log._id },
    ...
    return { type: job.type, status: 'failed', error: String(err?.message || err).slice(0, 500) };
  }
}
 ...
 * the in-flight lock). Pass { force: true } to also bypass the completed-window
 * dedup guard (used by scripts/run-job.js for manual / CI-triggered runs).
 * Returns the run outcome: { type, status: 'completed'|'failed'|'skipped', ... }.
 * Handy for manual ops / tests: require('./lib/jobScheduler').runJobNow('telegram-intel').
 */
async function runJobNow(type, { force = false } = {}) {
  const job = JOBS.find((j) => j.type === type);
  if (!job) throw new Error(`Unknown job type: ${type}`);
  return runJob(job, { force });
}
