...
// Resolve the Discord bot token + target guild for a channel, covering BOTH
// onboarding paths: the wizard (shared bot in botConfig.discord / env) and
// platformConnect (creator's own token in credenciales). Without this, wizard
// channels look token-less and the health check would falsely degrade them.
function resolveDiscordAccess(channel, creds) {
  const botToken = creds?.botToken || creds?.accessToken
    || channel.botConfig?.discord?.botToken || process.env.DISCORD_BOT_TOKEN || '';
  const serverId = channel.botConfig?.discord?.guildId
    || channel.identificadores?.serverId
...
async function fetchDiscord(channel) {
  const creds = getDecryptedCreds(channel);
  const { botToken, serverId } = resolveDiscordAccess(channel, creds);
...
async function verifyChannelAccess(channel) {
  ...
  try {
    switch (platform) {
      ...
      case 'discord': {
        const { botToken, serverId } = resolveDiscordAccess(channel, creds);
        // Missing shared bot token is OUR config problem, not the owner's —
        // flag as transient so the health check doesn't mass-degrade channels.
        if (!botToken) return { valid: false, error: 'Discord bot token no configurado', code: 'PLATFORM_DOWN' };
        if (!serverId) return { valid: false, error: 'Discord: servidor no configurado' };
        const discord = new DiscordAPI(botToken);
...
async function publishAdToChannel(channel, campaignOrContent, targetUrlArg) {
  ...
  switch (platform) {
    ...
    case 'discord': {
      // Wizard-onboarded channels use the shared bot (botConfig.discord/env);
      // the platformConnect path uses the creator's own token (credenciales).
      const botToken = creds.botToken || creds.accessToken
        || channel.botConfig?.discord?.botToken || process.env.DISCORD_BOT_TOKEN;
      // Publish target is the verified TEXT channel — NOT identificadorCanal,
      // which for Discord holds the guild id / a placeholder, never a channel.
      const channelId = channel.identificadores?.channelId;
      if (!botToken) throw new Error('Discord: falta el token del bot');
      if (!channelId) throw new Error('Discord: falta configurar el canal de publicación');
      const discord = new DiscordAPI(botToken);
