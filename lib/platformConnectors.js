...
async function fetchWhatsApp(channel) {
  const accessToken = channel.credenciales?.accessToken;
  const phoneNumberId = channel.credenciales?.phoneNumberId;
...
async function verifyChannelAccess(channel) {
  ...
  try {
    switch (platform) {
      ...
      case 'newsletter': {
        const apiKey = channel.credenciales?.accessToken;
...
async function publishAdToChannel(channel, content, targetUrl) {
  ...
  switch (platform) {
    case 'telegram': {
      const botToken = channel.credenciales?.botToken;
      const chatId = channel.identificadores?.chatId || channel.identificadorCanal;
      if (!botToken || !chatId) throw new Error('Telegram: credenciales incompletas');
      const telegram = new TelegramAPI(botToken);
    ...
    case 'whatsapp': {
      ...
      if (!accessToken || !phoneNumberId || !to) throw new Error('WhatsApp: credenciales incompletas');
      const whatsapp = new WhatsAppAPI(accessToken, phoneNumberId);
    ...
    case 'facebook': {
      const accessToken = channel.credenciales?.accessToken;
      const pageId = channel.identificadorCanal;
      if (!accessToken || !pageId) throw new Error('Facebook: credenciales incompletas');
      const facebook = new FacebookAPI();
      return facebook.publishPost(accessToken, pageId, content, targetUrl);
    }
    default:
