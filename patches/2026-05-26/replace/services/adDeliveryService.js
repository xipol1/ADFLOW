 ...
 *
 * Cambio respecto a la versión legacy:
 *   Antes: publishAdToChannel(channel, campaign.content, campaign.targetUrl)
 *   Ahora: publishAdToChannel(channel, campaign)  ← pasamos el documento
 *   entero para que el dispatcher tenga acceso al payload rico
 *   (media, buttons, embed, format). El dispatcher mantiene back-compat
 *   con la signature antigua, así que callers externos no se rompen.
 */
...
async function deliverAd(campaignId) {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      // Pasamos el documento completo a la dispatcher para que tenga
      // acceso al payload rico (media, buttons, embed). La signature
      // antigua sigue funcionando vía back-compat si algún caller externo
      // aún pasa (channel, content, targetUrl).
      const result = await publishAdToChannel(channel, campaign);
