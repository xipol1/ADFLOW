...
const autenticarPartner = async (req, res, next) => {
  try {
    ...
    const partner = await partnerIntegrationService.getPartnerByApiKey(apiKey);
    if (!partner) {
    ...
    partnerIntegrationService.touchPartnerUsage(partner._id, clientIp);
    return next();
  } catch (error) {
    return next(error);
  }
...
const limitadorPartner = rateLimit({
  ...
  keyGenerator: (req) => (req.partner?._id || req.partner?.id || req.ip).toString(),
...
const registrarActividadPartner = (req, res, next) => {
  ...
  res.json = (body) => {
    partnerIntegrationService.registerApiLog({
      partnerId: req.partner?._id || req.partner?.id,
      ip: getClientIp(req),
      method: req.method,
