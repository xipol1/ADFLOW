...
export default function AutoBuyPage() {
  ...
  if (launched && launchResult) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '50vh', flexDirection: 'column', gap: '20px', textAlign: 'center', fontFamily: F }}>
      ...
      <div>
        <h2 style={{ fontFamily: D, fontSize: '24px', fontWeight: 700, color: 'var(--text)', marginBottom: '8px' }}>
        ...
        <p style={{ fontSize: '14px', color: 'var(--muted)', maxWidth: '500px', lineHeight: 1.6 }}>
          ...
          un presupuesto total de <strong style={{ color: A }}>€{launchResult.totalBudget}</strong>.
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '16px', padding: '20px', width: '100%', maxWidth: '520px', textAlign: 'left' }}>
        <h3 style={{ fontFamily: D, fontSize: '14px', fontWeight: 600, color: 'var(--text)', marginBottom: '14px' }}>Canales asignados</h3>
          ...
          {(launchResult.campaigns || []).map((c, i) => (
            <div key={c._id || i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', background: 'var(--bg)', borderRadius: '10px', border: '1px solid var(--border)' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontFamily: D, fontSize: '15px', fontWeight: 700, color: A }}>€{c.price}</div>
      ...
      <div style={{ display: 'flex', gap: '10px' }}>
        <button onClick={() => { setLaunched(false); setLaunchResult(null); setStep(1); setAdText(''); setUrl('') }}
          style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '12px', padding: '12px 24px', fontSize: '14px', cursor: 'pointer', color: 'var(--text)', fontFamily: F }}>
          Nueva campaña
        </button>
        <button onClick={() => navigate('/advertiser/campaigns')}
          style={{ background: A, color: '#fff', border: 'none', borderRadius: '12px', padding: '12px 28px', fontSize: '14px', fontWeight: 600, cursor: 'pointer', fontFamily: F }}>
          Ver mis campañas
        </button>
      </div>
    </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 300px', gap: '20px', alignItems: 'start' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '16px', padding: '28px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: '8px', borderTop: '1px solid var(--border)' }}>
            <button onClick={() => step > 1 && setStep(1)} style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '10px', padding: '11px 22px', fontSize: '13px', cursor: step > 1 ? 'pointer' : 'default', color: step > 1 ? 'var(--text)' : 'var(--muted2)', fontFamily: F, opacity: step === 1 ? 0.4 : 1 }} disabled={step === 1}>
              ← Anterior
            </button>
            {step === 1
              ...
              : <button onClick={handleLaunch} disabled={!adText || !url.startsWith('http') || launching} style={{
                  background: adText && url.startsWith('http') && !launching ? A : 'var(--muted2)', color: '#fff', border: 'none', borderRadius: '10px',
                  padding: '11px 24px', fontSize: '13px', fontWeight: 600,
                  cursor: adText && url.startsWith('http') && !launching ? 'pointer' : 'not-allowed',
                  fontFamily: F, boxShadow: adText && url.startsWith('http') && !launching ? `0 4px 14px ${AG(0.35)}` : 'none',
        ...
        <div style={{ position: 'sticky', top: '20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          {/* Info card */}
          <div style={{ background: AG(0.06), border: `1px solid ${AG(0.2)}`, borderRadius: '16px', padding: '18px' }}>
            <h3 style={{ fontFamily: D, fontSize: '13px', fontWeight: 600, color: A, marginBottom: '8px' }}>¿Cómo funciona?</h3>
