...
// ─── Colors for charts ──────────────────────────────────────────────────────
const PIE_COLORS = [C.teal, C.adv, C.cre, C.gold, '#EF4444', '#3B82F6', '#EC4899', C.silver]
  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, gap: 12, flexWrap: 'wrap' }}>
        <div>
          <h3 style={{ color: C.t1, fontSize: 15, fontWeight: 700, margin: 0, fontFamily: D }}>{title}</h3>
...
function ChartTooltipContent({ active, payload, label }) {
  ...
  return (
    <div style={{ background: C.surfaceEl, border: `1px solid ${C.borderEl}`, borderRadius: 8, padding: 12, fontFamily: 'JetBrains Mono, monospace', fontSize: 12 }}>
      <div style={{ color: C.t2, marginBottom: 6 }}>{label}</div>
      {payload.map((p) => (
        <div key={p.dataKey} style={{ display: 'flex', justifyContent: 'space-between', gap: 16, color: p.color || C.t1 }}>
  ...
  return (
    <SectionCard title="Rendimiento de campañas">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12, marginBottom: 24 }}>
        <KPICard label="Gasto total" valor={fmtMoney(totalSpend)} icon={<DollarSign size={16} />} color={C.teal} />
        ...
        <KPICard label="Tasa éxito" valor={filtered.length > 0 ? `${Math.round((completed.length / filtered.length) * 100)}%` : '--'} color={completed.length > 0 ? C.ok : C.t2} icon={<TrendingUp size={16} />} />
      </div>

      {/* Monthly spend chart */}
      {barData.length > 1 && (
        <div style={{ marginBottom: 24 }}>
          <div style={{ color: C.t3, fontSize: 11, marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600 }}>
            Gasto mensual
          </div>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={barData} margin={{ top: 8, right: 8, left: -16, bottom: 0 }}>
              <CartesianGrid stroke={C.border} strokeDasharray="3 3" />
              <XAxis dataKey="label" stroke={C.t3} tick={{ fontSize: 11, fill: C.t3 }} tickLine={false} axisLine={{ stroke: C.border }} />
              <YAxis stroke={C.t3} tick={{ fontSize: 11, fill: C.t3 }} tickLine={false} axisLine={{ stroke: C.border }} tickFormatter={(v) => `€${v}`} />
              <Tooltip content={<ChartTooltipContent />} cursor={{ fill: `${C.teal}11` }} />
              <Bar dataKey="gasto" name="Gasto" fill={C.teal} radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
        {filtered.slice(0, 10).map((c) => {
          ...
          const statusColor = STATUS_COLORS[c.status] || C.t3
          return (
            <div
              key={c._id}
              style={{
            ...
            >
              ...
              <span style={{ color: C.t1, fontWeight: 600, flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {chName}
              </span>
              <span className="font-mono" style={{ color: C.t2, flexShrink: 0 }}>
                {fmtMoney(c.price)}
              </span>
              ...
              <span style={{ color: C.t3, flexShrink: 0, fontSize: 11 }}>
                {fmtDate(c.createdAt)}
              </span>
            </div>
          )
...
function SpendAnalysisSection({ campaigns }) {
  ...
  const PieLabel = ({ name, value, cx, cy, midAngle, outerRadius }) => {
    ...
    return (
      <text x={x} y={y} fill={C.t2} textAnchor={x > cx ? 'start' : 'end'} dominantBaseline="central" style={{ fontSize: 11, fontFamily: 'JetBrains Mono, monospace' }}>
        {name}
      </text>
    )
  }
  ...
  return (
    <SectionCard title="Análisis de gasto" subtitle={`${completed.length} campañas completadas · ${fmtMoney(totalSpend)} total`}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 24, marginBottom: 20 }}>
        ...
        {byPlatform.length > 0 && (
          <div>
            <div style={{ color: C.t3, fontSize: 11, marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600 }}>
              Por plataforma
            </div>
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={byPlatform} cx="50%" cy="50%" outerRadius={70} innerRadius={40} dataKey="value" label={PieLabel} labelLine={false} strokeWidth={2} stroke={C.bg}>
                  {byPlatform.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                </Pie>
                <Tooltip formatter={(v) => fmtMoney(v)} contentStyle={{ background: C.surfaceEl, border: `1px solid ${C.borderEl}`, borderRadius: 8, fontSize: 12 }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
        )}
        ...
        {byNiche.length > 0 && (
          <div>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={byNiche} cx="50%" cy="50%" outerRadius={70} innerRadius={40} dataKey="value" label={PieLabel} labelLine={false} strokeWidth={2} stroke={C.bg}>
      ...
      {/* Summary stats */}
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', padding: '16px 0', borderTop: `1px solid ${C.border}` }}>
        <div>
          <div style={{ color: C.t3, fontSize: 10, textTransform: 'uppercase', marginBottom: 2 }}>Canal más usado</div>
          <div className="font-mono" style={{ color: C.t1, fontSize: 13, fontWeight: 600 }}>{topChannel}</div>
        </div>
        <div>
          <div style={{ color: C.t3, fontSize: 10, textTransform: 'uppercase', marginBottom: 2 }}>Nicho principal</div>
          <div className="font-mono" style={{ color: C.t1, fontSize: 13, fontWeight: 600 }}>{byNiche[0]?.name || '--'}</div>
        </div>
        <div>
          <div style={{ color: C.t3, fontSize: 10, textTransform: 'uppercase', marginBottom: 2 }}>CPM medio</div>
          <div className="font-mono" style={{ color: C.teal, fontSize: 13, fontWeight: 600 }}>
      ...
      {creating && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
          <input
            ...
            autoFocus
            style={{
              flex: 1, background: C.surfaceEl, border: `1px solid ${C.borderEl}`,
              borderRadius: 8, padding: '8px 12px', fontSize: 13,
              color: C.t1, outline: 'none', fontFamily: F,
            }}
          />
          <button
          ...
          <button
            ...
            style={{ background: 'transparent', border: `1px solid ${C.border}`, borderRadius: 8, padding: '8px', cursor: 'pointer', color: C.t3, display: 'flex' }}
          >
            <X size={14} />
          </button>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 160, color: C.t3, gap: 8 }}>
          ...
          <button
            ...
            style={{
              ...
              border: `1px solid ${C.teal}66`, borderRadius: 8,
              padding: '6px 14px', fontSize: 12, fontWeight: 600,
                    ...
                    return (
                      <div key={chId} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        {isPopulated ? (
                          ...
                          <div style={{ flex: 1, color: C.t2, fontSize: 12, fontFamily: 'JetBrains Mono, monospace' }}>
                        ...
                        <button
                          ...
                          style={{
                            ...
                            color: C.t3, cursor: 'pointer', padding: 4,
                            flexShrink: 0,
                          }}
                        >
                          <X size={12} />
